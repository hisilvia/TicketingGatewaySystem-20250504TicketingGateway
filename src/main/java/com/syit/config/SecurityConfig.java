...
public class SecurityConfig {
	...
	public SecurityFilterChain apiFilterChain2(HttpSecurity http) throws Exception {
		http
			...
			.flag(true).and()
			.authorizeRequests()
				.requestMatchers("/home","/html/**").permitAll().and()
			      .exceptionHandling().accessDeniedPage("/accessDeniedPage").and()
			.authorizeRequests()
				.requestMatchers("/home","/user/**", "/userProfile").hasAnyAuthority("USER")
				.requestMatchers("/manager/**","/userProfile").hasAnyAuthority("MANAGER")	
				.requestMatchers("/admin/**").hasAnyAuthority("ADMIN").and()
		.formLogin()
			.loginPage("/login")
